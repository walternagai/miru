        ...
        async with OllamaClient(host) as client:
            try:
                ...
                if destination in model_names and not force:
                    render_error(
                        t("error.model_exists", model=destination),
                        t("suggestion.force_overwrite"),
                    )
                    raise typer.Exit(code=1)
            ...
            try:
                ...
                if model_info:
                    ...
                    render_success(
                        f"{t('success.model_copied', new_name=destination)} ({size_gb:.2f} GB)"
                    )
                else:
                    render_success(t("success.model_copied", new_name=destination))
